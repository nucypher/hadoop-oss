 ...
 */
public class KeyAuthorizationKeyProvider extends KeyProviderProxyReEncryptionExtension {

  public static final String KEY_ACL = "key.acl.";
  private static final String KEY_ACL_NAME = KEY_ACL + "name";

  public enum KeyOpType {
    ALL, READ, MANAGEMENT, GENERATE_EEK, DECRYPT_EEK, GENERATE_REK;
  }
  ...
  private final KeyProviderProxyReEncryptionExtension provider;
  ...
  public KeyPairVersion createKeyPair(String name, Options options)
    ...
    try {
      ...
      return provider.createKeyPair(name, options);
    } finally {
      writeLock.unlock();
    }
  }
  ...
  @Override
  public KeyPairVersion rollNewVersionPair(String name) throws NoSuchAlgorithmException, IOException {
    writeLock.lock();
    try {
      doAccessCheck(name, KeyOpType.MANAGEMENT);
      return provider.rollNewVersionPair(name);
    } finally {
      writeLock.unlock();
    }
  }
  ...
  public byte[] generateReEncryptionKey(String encryptionKeyName, String destinationKeyVersionName, KeyPairMaterial destinationKey)
    ...
    readLock.lock();
    try {
      KeyVersion keyVersion = provider.getKeyVersion(encryptionKeyName);
      if (keyVersion != null) {
        doAccessCheck(keyVersion.getName(), KeyOpType.GENERATE_REK);
      }
      return provider.generateReEncryptionKey(encryptionKeyName, destinationKeyVersionName, destinationKey);
    } finally {
      readLock.unlock();
    }
  }
  ...
  public byte[] generateReEncryptionKey(String encryptionKeyVersionName, String destinationKeyVersionName)
    ...
    readLock.lock();
    try {
      KeyVersion keyVersion = provider.getKeyVersion(encryptionKeyVersionName);
      if (keyVersion != null) {
        doAccessCheck(keyVersion.getName(), KeyOpType.GENERATE_REK);
      }
      keyVersion = provider.getKeyVersion(destinationKeyVersionName);
      if (keyVersion != null) {
        doAccessCheck(keyVersion.getName(), KeyOpType.GENERATE_REK);
      }

      return provider.generateReEncryptionKey(encryptionKeyVersionName, destinationKeyVersionName);
    } finally {
      readLock.unlock();
    }
  }
