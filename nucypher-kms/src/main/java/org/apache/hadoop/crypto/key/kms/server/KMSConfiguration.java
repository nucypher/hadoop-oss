...
public class KMSConfiguration {
  ...
  public enum Mode {
    GLOBAL_KMS, LOCAL_KMS, RE_NODE;
    public static Mode convert(String name) {
      Mode[] modes = Mode.values();
      for (Mode suite : modes) {
        if (suite.toString().equals(name)) {
          return suite;
        }
      }
      throw new IllegalArgumentException("Invalid mode name: " + name);
    }
  }

  public static final String KMS_MODE_KEY = CONFIG_PREFIX +
      "mode";
  public static final String KMS_MODE_DEFAULT = Mode.GLOBAL_KMS.toString();
