...
function hadoop_subproject_init
{
  ...
  if [[ -z "${HADOOP_KMS_ENV_PROCESSED}" ]]; then
    if [[ -e "${NUCYPHER_KMS_CONF_DIR}/kms-env.sh" ]]; then
      . "${NUCYPHER_KMS_CONF_DIR}/kms-env.sh"
  ...
  export HADOOP_CATALINA_TEMP="${KMS_TEMP:-${NUCYPHER_KMS_PREFIX}/temp}"

  hadoop_deprecate_envvar KMS_CONFIG HADOOP_CONF_DIR

  hadoop_deprecate_envvar KMS_LOG HADOOP_LOG_DIR

  export HADOOP_CATALINA_CONFIG="${NUCYPHER_KMS_CONF_DIR}"
  ...
  export CATALINA_BASE="${CATALINA_BASE:-${NUCYPHER_KMS_PREFIX}/share/hadoop/kms/tomcat}"
...
elif [[ -e "${NUCYPHER_KMS_PREFIX}/libexec/hadoop-config.sh" ]]; then
  . "${NUCYPHER_KMS_PREFIX}/libexec/hadoop-config.sh"
else
