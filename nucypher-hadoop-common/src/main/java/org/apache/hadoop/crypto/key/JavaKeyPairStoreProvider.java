...
/**
 * KeyPairProvider based on Java's KeyStore file format. The file may be stored in
...
public class JavaKeyPairStoreProvider extends KeyPairProvider {
  private static final String KEY_METADATA = "KeyMetadata";
  private static Logger LOG =
      LoggerFactory.getLogger(JavaKeyPairStoreProvider.class);

  public static final String SCHEME_NAME = "jcekps";
  ...
  @VisibleForTesting
  JavaKeyPairStoreProvider(JavaKeyPairStoreProvider other) {
  ...
  private JavaKeyPairStoreProvider(URI uri, Configuration conf) throws IOException {
    super(conf);
    this.uri = uri;
    ...
    try {
      ...
      keyStore = KeyStore.getInstance(JavaKeyStoreProvider.SCHEME_NAME);
      ...
      throw new IOException("Can't create keypairstore", e);
  ...
  public KeyVersion getKeyVersion(String versionName) throws IOException {
    ...
    try {
      ...
      try {
        ...
        keyPair = (KeyPairSpec) keyStore.getKey(versionName, password);
        privateKey = keyPair.getPrivate();
      } catch (KeyStoreException e) {
        throw new IOException("Can't get key " + versionName + " from " +
            path, e);
      } catch (NoSuchAlgorithmException e) {
        throw new IOException("Can't get algorithm for key " + privateKey + " from " +
            path, e);
      } catch (UnrecoverableKeyException e) {
        throw new IOException("Can't recover key " + privateKey + " from " + path, e);
      }
      return new KeyPairVersion(getBaseName(versionName), versionName,
          new KeyPairMaterial(null, privateKey.getEncoded())).privateToKeyVersion();
    } finally {
      readLock.unlock();
    }
  }
  ...
  KeyVersion innerSetKeyVersion(String name, String versionName, byte[] material,
    ...
    try {
      keyStore.setKeyEntry(versionName, new KeyPairSpec(null, material, cipher),
          password, null);
    } catch (KeyStoreException e) {
  ...
  KeyPairVersion innerSetKeyVersion(String name, String versionName, KeyPairMaterial material,
                                String cipher) throws IOException {
    try {
      // TODO rid of PrivateKey here and make some KeyPairKeySpec, because don't want to
      // fight with certificates, which we cannot sigb
      keyStore.setKeyEntry(versionName, new KeyPairSpec(material.getPublic(), material.getPrivate(), cipher),
          password, null);
    } catch (KeyStoreException e) {
      throw new IOException("Can't store key " + versionName + " in " + this,
          e);
    ...
    changed = true;
    return new KeyPairVersion(name, versionName, material);
  }
  ...
  public KeyPairVersion getKeyPairVersion(String versionName) throws IOException {
    ...
    try {
      ...
      return new KeyPairVersion(getBaseName(versionName), versionName,
          new KeyPairMaterial(publicKey.getEncoded(), privateKey.getEncoded()));
    } finally {
      readLock.unlock();
    }
  }
  ...
  public List<KeyPairVersion> getKeyPairVersions(String name) throws IOException {
    readLock.lock();
    try {
      List<KeyPairVersion> list = new ArrayList<KeyPairVersion>();
      Metadata km = getMetadata(name);
      if (km != null) {
        int latestVersion = km.getVersions();
        KeyPairVersion v = null;
        String versionName = null;
        for (int i = 0; i < latestVersion; i++) {
          versionName = buildVersionName(name, i);
          v = getKeyPairVersion(versionName);
          if (v != null) {
            list.add(v);
          }
        }
  ...
  @Override
  public KeyPairVersion createKeyPair(String name, KeyPairMaterial material, Options options) throws IOException {
    Preconditions.checkArgument(name.equals(StringUtils.toLowerCase(name)),
        "Uppercase key names are unsupported: %s", name);
    writeLock.lock();
    try {
      ...
      Metadata meta = new Metadata(options.getCipher(), options.getBitLength(),
          options.getDescription(), options.getAttributes(), new Date(), 1);
      /* don't need this assert for complex keys
      TODO store G in some more apropriate way
      if (options.getBitLength() != 8 * material.getPrivate().length) {
        throw new IOException("Wrong key length. Required " +
            options.getBitLength() + ", but got " + (8 * material.getPrivate().length));
      }*/
      cache.put(name, meta);
      String versionName = buildVersionName(name, 0);
      return innerSetKeyVersion(name, versionName, material, meta.getCipher());
  ...
  public static class Factory extends KeyProviderFactory {
    ...
    public KeyProvider createProvider(URI providerName,
      ...
      if (SCHEME_NAME.equals(providerName.getScheme())) {
        return new JavaKeyPairStoreProvider(providerName, conf);
      }
      return null;
    }
  }
