 ...
 */
public abstract class KeyPairProviderExtension
<E extends KeyProviderExtension.Extension> extends KeyPairProvider {
  private KeyProvider keyProvider;
  private E extension;

  public KeyPairProviderExtension(KeyProvider keyProvider, E extensions) {
  ...
  public KeyPairVersion getKeyPairVersion(String versionName) throws IOException {
    return getKeyPairProvider().getKeyPairVersion(versionName);
  }

  @Override
  public List<KeyPairVersion> getKeyPairVersions(String name) throws IOException {
    return getKeyPairProvider().getKeyPairVersions(name);
  }

  @Override
  public KeyPairVersion getCurrentKeyPair(String versionName) throws IOException {
    return getKeyPairProvider().getCurrentKeyPair(versionName);
  }

  @Override
  public KeyPairVersion createKeyPair(String name, KeyPairMaterial material, Options options)
      throws IOException, NoSuchAlgorithmException {
    return getKeyPairProvider().createKeyPair(name, material, options);
  }

  @Override
  public KeyPairVersion rollNewVersionPair(String name, KeyPairMaterial material) throws IOException {
    return getKeyPairProvider().rollNewVersionPair(name, material);
  }

  public KeyPairProvider getKeyPairProvider() {
    if (!(keyProvider instanceof KeyPairProvider)) {
      throw new ClassCastException("keyProvider type is not a KeyPairProvider");
    }
    return (KeyPairProvider) keyProvider ;
  }
